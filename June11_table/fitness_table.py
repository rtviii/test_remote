...
for exp_type in range(1,12):
    ...
    for file in glob.glob(f"/home/rxz/dev/polygenicity-simulations/{folder}/exp{exp_type}/exp{exp_type}/fitness_data/*.parquet"):
    ...
    for file in glob.glob(f"/home/rxz/dev/polygenicity-simulations/{folder}/exp{exp_type+11}/exp{exp_type+11}/fitness_data/*.parquet"):
        data       = pd.read_parquet(file)
        lastthird  = data['fit'][   int( len(data['fit']) - len(data['fit'])/3 ):]
