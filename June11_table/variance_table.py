...
for exp_type in range(1,12):
    ...
    for file in glob.glob(f"/home/rxz/dev/polygenicity-simulations/staticpopv5/exp{exp_type}/exp{exp_type}/var_covar/*.json"):
    ...
    for file in glob.glob(f"/home/rxz/dev/polygenicity-simulations/staticpopv5/exp{exp_type+11}/exp{exp_type+11}/var_covar/*.json"):
        with open (file ) as infile:
            data = json.load(infile)
            var  = data['variance']
            cov  = data['covariance']
...
variance_sheet.to_csv('variance_sheet.csv')
covariance_sheet.to_csv('covariance_sheet.csv')
